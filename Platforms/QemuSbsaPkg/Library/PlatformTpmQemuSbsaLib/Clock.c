...
// Ticks per ms for the SBSA generic timer.
#define CLOCK_SBSA_NOMINAL  (ArmGenericTimerGetTimerFreq () / 1000U)

// 0.5% change in rate.
#define CLOCK_SBSA_ADJUST_COARSE  (CLOCK_SBSA_NOMINAL / 200)

// 0.05% change in rate.
#define CLOCK_SBSA_ADJUST_MEDIUM  (CLOCK_SBSA_NOMINAL / 2000)

// A minimum change in rate.
#define CLOCK_SBSA_ADJUST_FINE    1

// The clock tolerance is +/- 10%.
#define CLOCK_SBSA_ADJUST_LIMIT   ((CLOCK_SBSA_NOMINAL / 100) * 10)
...
/**
  Read current timer value and return the elapsed ticks since the previous read.

  @return Delta value since last timer read.
  ...
  )
{
  ...
  mAdjustTimerRate = CLOCK_SBSA_NOMINAL;
  mTimerWasReset   = TRUE;
...
{
  // This interface is for the simulator path and should not be called.
  PANIC ("TimerRestart should not be called in this platform");
  return;
}
...
{
  // This interface is for the simulator path and should not be called.
  PANIC ("RealTime should not be called in this platform");
  return 0;
}
  ...
  )
{
  ...
  if (mLastSystemTimer == 0x00) {
    ...
    Diff              = GetTimerDiff ();
    AdjustedTimerDiff = (Diff * CLOCK_SBSA_NOMINAL) / mAdjustTimerRate;
  ...
  )
{
  ...
  if (mAdjustTimerRate > (CLOCK_SBSA_NOMINAL + CLOCK_SBSA_ADJUST_LIMIT)) {
    mAdjustTimerRate = CLOCK_SBSA_NOMINAL + CLOCK_SBSA_ADJUST_LIMIT;
  }
